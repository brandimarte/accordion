...
svg.setAttribute("height", spacingY * rowTypes.length + 80); // Increased height for padding
...
const verticalOffset = 30; // Distance from the top edge (increased)
const horizontalPadding = 40; // Distance from left/right edges (increased)
...
function highlightSingleChord(rootNote, chordType, className) {
  ...
  if (chordType === "major") {
  ...
  } else if (chordType === "dim_triad") {
    const unplayable_dim_roots = ["Cb", "Fb", "Bbb"];
    if (unplayable_dim_roots.includes(rootNote)) {
      return; // Don't highlight these unplayable chords
    }

    const minorThirdIndex = (rootNoteIndex + 3) % 12;
    const enharmonicMinorThirds = indexToAllNotes[minorThirdIndex] || [];
    const rootNoteBassIndex = bassNotes.indexOf(rootNote);

    let bestFitMinorThird = null;
    let minDistance = Infinity;

    for (const note of enharmonicMinorThirds) {
    ...
    // If the note in the next column is not the fifth, do nothing.
  }
}
